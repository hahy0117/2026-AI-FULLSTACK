class A11 {
    int a; // (1)  인스턴스 변수-heap area-new -this 각각
    ...
    //(3) void show()
    void show() { 인스턴스메서드-heap area-new 생성자-this 각각
        this.a = 11; 
...
Q3. (b)번 위치에서 All a1 = new All();

메모리 빌려오고,객체 생성하는 역할 :(new)
String 은 null,int는 0으로 초기화하는 역할 : ( All())
new All()한 주소를 갖고 있는 것은 : (참조변수 a1)

Q4. 기본생성자를 반드시 선언해야하는 경우를 적으시오.
다른 생성자를 작성시 ,상속

Q5. 다음 중 오버로딩이 성립하기 위한 조건이 아닌 것은? (모두 고르시오)
a. 메서드의 이름이 같아야 한다. -> 오버로딩 조건 맞다
b. 매개변수의 개수나 타입이 달라야 한다.-> 오버로딩 조건 맞다
c. 리턴타입이 달라야 한다.-> 오버로딩 조건 아니다 정답
d. 매개변수의 이름이 달라야 한다.->오버로딩 조건 아니다 정답


Q6. 다형성이란? 
하나의 타입으로 여러 형태를 사용 할 수 있는 성질
