...
-package.json
{
  ...
  "scripts": {
    ...
    "test": "jest" // jest 활용해서 테스트 코드 실행 했다고 면접에서 답변
  },
  "author": "sally",
  "license": "ISC",
  "dependencies": {
    "@ant-design/icons": "^4.3.0",  //ant design ul 라이브러리
...
├── .next/                  # ✅ Next.js 빌드 결과물 (자동 생성, 배포 시 사용)
├── node_modules/           # ✅ 설치된 npm 패키지들
├── pages/                  # ✅ Next.js 라우팅 기반 페이지 폴더 (이름 바꾸면 안됌 기본으로 설정 되어있는 파일 pages)
│   └── index.js            # 메인 페이지 
├── package-lock.json       # npm 의존성 잠금 파일
└── package.json            # 프로젝트 메타 정보 및 의존성 
```
...
설정 파일
front/
├── .babelrc  //babel 설정 파일 :jsx->일반 변환 브라우저 이해할 수 있게,
...
├── components/             # ✅ 재사용 가능한 UI 컴포넌트 폴더
│   └── Layout.js           # 페이지 공통 레이아웃 컴포넌트
├── pages/                  # ✅ Next.js 빌드 결과물 (자동 생성, 배포 시 사용)
│   ├── _app.js             # 전체 앱의 공통 설정 (Redux Provider, 글로벌 스타일 등)
│   ├── index.js            # 메인 페이지

[componets]-Layout.js 공통틀(해더,네이게이션,푸터)
[pages] -app.js 모든페이지들을 Layout 감싸는 진입점
[pages] - index.js 본문 콘텐츠

3.app.js Layout 감싸서 영역안에 {children}
step1) localhost:3000 접속
step2) Layout에서의 해더,네이게이션,푸터 랜더링
...
│   ├── index.js      #메인페이지
    ...
    - 상태조회: 스토어저장된 전역상태(State) react 에서 가져오기
