...
Robot (String name)
↑
CleaningRobot( name/@work/vacuum()),CookingRobot( name,@work(),cook()}, FightingRobot( name/@work()/attack()}

1)부모     =자식(업캐스팅)
2){name="기본로봇"/work()} =1번지{name/@work/vacuum()} -{name="기본로봇"/work()}
                        =2번지{ name,@work(),cook()}-{name="기본로봇"/work()}
                        =3번지{name="전투로봇"/work()/attack()}-{name="기본로봇"/work()}
 
 * */

//Q2. 각 클래스에서 사용할 수 있는 멤버변수/
...
abstract class Robot {
    String name = "기본로봇";
    public Robot() { super(); }
    public abstract void work(); // 추상 메서드 {System.out.println("로봇일?")}
} // end class
