...
//2.o:개방페쇄 (새로운쿠키 추가가능,기존코드 건들지말기)
interface Cookie {
	void make();
}
...
class  ChocoBananaCookie implements Cookie{
	...
	@Override
	public void make() {
		System.out.println("초코바나나쿠기");		
	}
	
}

//3. L: 리스코프지환 (어떤 쿠키든 Cookie 인터페이스로 바꿔써도 문제없어요 부모=자식)
class CookieFactory{
	//Cookie cookie 각종 쿠키 종류
...
//            CookieFactory는 구체적인 쿠기가 아니라 Cookie에 의존
class CookeShop{
...
1. S :한클래스는 하나만(한 가지 일만/쿠키 반죽 반죽만,쿠키굽기는 쿠키굽기만)
2. O :(수정말고 추가만) 새로운 쿠기 쉽게 추가
3. L :(다향성: 같은방식으로  처리) 모든쿠키는 같은 방식으로 만들기
