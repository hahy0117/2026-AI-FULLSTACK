...
class Farm{
	//상태(멤버변수)
	String name; //인스턴스변수-heap area - new()-생성자
	int age;
...
public class Class005 {
	...
	public static void main(String[] args) {
		...
		System.out.println("\n\n1.동물식구-this-각각");
		Farm cat=new Farm();//1)new 객체 만들기 2)Farm() 초기화 3)cat번지
		cat.name="kitty"; cat.age=6; cat.numPlus(); cat.show();
		
		
		Farm dog=new Farm();//1)new 객체 만들기 2)Farm() 초기화 3)cat번지
		dog.name="하루"; dog.age=6; dog.numPlus(); dog.show();
...
[METHOD:정보] static,final,Farm.class,Class005.class
            Farm.FarmName="(주) 동물농장"
------------------------------------
[HEAP:동적]            |  [STACK:지역]

2번지{name=null,age=0;} <- cat [2번지]
1번지{name="kitty",age=0}  <- cat[1번지]
