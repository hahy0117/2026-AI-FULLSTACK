...
class Boot1ApplicationTests2_service {
	...
	void createTest() {
		...
		AppUser user = AppUser.builder().email("z@z").password("z").nickname("first").provider("local").build();
	...
	void testUpdatePost() {
		...
		assertThat(created.getId()).isNotNull();
		// 그 게시글 수정
		Post updated = postService.updatePost(created.getId(), "수정");
	...
	void testDeletePost() {
		...
		boolean exists = posts.stream().anyMatch(post -> post.getId().equals(created.getId()));
