...
//store : useSelect(전역) ,useDispatch(스토어이벤트알림) / 감지: useEffect(이벤트변경 감지),useState(변수) / 경로 : useRouter
...
//2. function (부품)
function   SignupPage(){
    //5개 부품 -useEffect(이벤트변경감지),useState(변수)
    const dispatch=useDispatch(); //이벤트 변경 감지
    const router=useRouter();//경로
    const {use,error,success,loading}=useSelector((state)=>state.auth);
    ...
    return (<Row  justify="center"  style={{ padding:"20px"}} >
        ...
        {!success && (
          ...
          <Form layout="vertical" onFinish={onFinish}>
            ...
            {/** 이메일+중복검사 Form.Item > Input /name ,hasFeedback 아이콘  */}
