...
delete from select_userinfo where no=8 or no=9 or no=10;
update select_userinfo set age=55 where no=5;
update select_userinfo set name='third',age=66 where no=6;
update select_userinfo set name='seven',age=null where no=7;
...
-- Q1. 나이가 많은 순으로 출력
 select*from select_userinfo order by age asc; -- 오름차순  where 생략 가능
 select*from select_userinfo order by age desc; -- desc 내림차순
-- Q2. 나이가 많은 순으로 2명 출력
select no,age
...
select no,age
from select_userinfo
order by age desc limit 0,2;-- 어디서부터,몇개 (최신부터,2개) 66 55

select no,age
from select_userinfo
order by age desc limit 1,2; -- 어디서부터, 몇개 (최신부터,2개) 55 44
...
select empno 'EMPLOYEE_NO', ename 'EMPLOYEE_NAME',job 'JOB', mgr 'MANAGER', hirdate 'HIREDATE', sal 'SALARY', comm  'COMMISSION', dept 'DEPARTMENT_NO' 
from emp
