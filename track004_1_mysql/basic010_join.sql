...
desc dept;
desc emp;
alter table emp change hirdate hiredate date;
...
-- 0	65	11:13:33	SELECT EMPNO, ENAME, DEPTNO, DNAME, LOC
--   FROM EMP E, DEPT D
--   WHERE E.DEPTNO = D.DEPTNO
...
order by grade,sal; -- asc 오름차순은 안 적어도 기본으로 정렬된다
-- +-------+--------+-----------+------+------------+------+------+--------+-------+-------+-------+
...
order by mgr.empno ASC; -- 직속상관의 번호를 오름차순으로
-- +-------+--------+------+-----------+-----------+
...
-- -- 사원의 직속상관(MGR)의 정보를 다음과 같이 조회하시오.
select sawon.empno , sawon.ename , sawon.mgr  , mgr.empno `MGR_EMPNO`  , mgr.ename `MGR_ENAME`
from  emp sawon  , emp mgr
where  sawon.mgr=mgr.empno
...
from emp e right join dept d on(e.deptno=d.deptno) left join salgrade s on(e.sal between s.losal and s.hisal) left join emp mgr
on(e.mgr=mgr.empno)
order by d.deptno,e.ename;

-- natural join 

select deptno,dname,empno,ename,mgr,sal,deptno,
losal,hisal,grade,
mgr.empno 'MGR_EMPNO',mgr.ename'MGR_ENAME'
from emp e left join dept d on(e.deptno=d.deptno) left join salgrade s on(e.sal between s.losal and s.hisal) left join emp mgr
on(e.mgr=mgr.empno)
