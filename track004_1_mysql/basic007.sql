...
select ban ,count(1) from  group_userinfo group by ban;
...
select max(sal) '부서10 최대급여' from emp group by dept having dept=10;
...
select hirdate as '부서20 최근입사일'
from emp
order by hirdate desc
limit 1;
...
select hirdate as '부서20 제일오래 된 입사일'
from emp
