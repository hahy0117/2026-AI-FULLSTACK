...
insert into userinfo_select values (2,'name',22);
...
update userinfo_select set no=10 where no=4;
select * from userinfo_select;
...
-- select*from userinfo userinfo_select
...
-- = BASIC 
select ceil(1.1) '올림 2', floor(1.9) '내림',round(1.5) '반올림',mod(10,3)'나머지' from dual;

select 1.2345,
round(1.2545,1),-- 소수점 1번째 자리
round(15.2345,-1) -- 정수 1번째 자리
from dual;
-- +-----------+------------+------------+-----------+
...
select substr("abcd",2.2 ), -- 두번째에서 부터 두개 선택
substr("abcd",2.3),
substr("abcd",1.3)
from dual;
-- +------+------+------+
...
select concat('choco','milk') from dual; -- 두 문자를 이어주는 역할
-- +--------------------------+
-- | concat('choco' , 'milk') |
-- +--------------------------+
-- | chocomilk                |
...
select concat('#', trim(' a b c ')  ,'#' ) -- 양쪽공백
	,concat('#', ltrim(' a b c ')  ,'#' )-- 왼쪽공백
...
alter table fn_select_userinfo2 modify name varchar(20);
alter table fn_select_userinfo2 add email varchar(20);
-- +-------+-------------+------+-----+---------+-------+
-- | Field | Type        | Null | Key | Default | Extra |
-- +-------+-------------+------+-----+---------+-------+
-- | no    | int         | NO   |     | 0       |       |
...
insert into fn_select_userinfo2 values (6,'bca',666,'bca@gmail.com');
...
-- select concat('choco','milk') from dual; -- 두 문자를 이어주는 역할
select concat(name,'개발자입니다')' 직업' from fn_select_userinfo2;
...
create table select_userinfo2 select*from select_userinfo where 1=2;
select * from select_userinfo2;
desc select_userinfo2;
alter table select_userinfo2 add email varchar(20);
-- +----+-------+-----+---------------+
-- | no | name  | age | email         |
-- +----+-------+-----+---------------+
...
insert into select_userinfo2 values(2,'bbbb',22,'bbb@gmail.com');
