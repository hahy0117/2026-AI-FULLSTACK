...
select num from atest where num in(3,4,5);
...
select num
from atest
where num<all(select num from atest where num in(3,4,5) ) -- all 모두 최소 3 보다 작은 값
order by num;

-- Q4)
select num 
from atest
where num>all (select num from atest where num in(3,4,5) ) -- all 최대 5보다 큰 값
order by num;
-- --------------------------------------------------------
-- --------------------------------------------------------
-- -- 연습문제-9
...
-- +-------+-------+-----------+------+------------+------+------+--------+
-- 4 rows in set (0.00 sec)
-- emp 테이블에서 any 이용하여 30번 부서의 최고 급여보다 작은 급여 (최고급여보다 작은)
